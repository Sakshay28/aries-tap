// The permanent resolver, shared by both physical media.
//
// A printed QR and a written NFC tag point at the same record and resolve
// identically — only the `medium` differs, so the dashboard can tell a tap from
// a scan. This is the reason a printed Aries Tap tag never has to be reprinted:
//
...
export type TapMedium = "qr" | "nfc";

/**
 * Resolve a scanned/tapped code to its current destination.
 *
 * `medium` changes nothing about routing or attribution — the same code is the
 * same table either way. It is recorded so an owner can see whether guests are
 * actually tapping the NFC or falling back to the QR, which is the only way to
 * find out that a batch of tags was never programmed.
 */
      ...
      await ingestEvent(
        {
          ...
          sessionId: anonId,
          source: "resolver",
          meta: { label: qr.label, medium, table: qr.table, referer: referer.slice(0, 160) },
          idempotencyKey: `${medium}:${anonId}:${qr.code}:${Math.floor(Date.now() / 5000)}`,
        },
        req.headers,
        {
          rateLimit: false,
